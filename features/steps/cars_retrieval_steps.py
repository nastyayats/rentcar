...
def step_impl(context, car_id):
    context.response = send_car_retrieval_request(car_id, context.r_token)
    context.car_id = car_id


@when('send request to retrieve car')
def step_impl(context):
    context.response = send_car_retrieval_request(context.car_id, context.r_token)


@when('send request to retrieve cars list')
def step_impl(context):
    context.response = send_cars_list_retrieval_request(context.r_token)


@then('response for car retrieval request is valid')
def step_impl(context):
    validatejson.successful_car_retrieval_response(context.response.json())
...
def step_impl(context, number):
    validatejson.successful_cars_list_retrieval_response(context.response.json())
    data = context.response.json().get('data')
